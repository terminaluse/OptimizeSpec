...
EVIDENCE_REQUIRED_ARTIFACTS = [
    ...
    "evidence/evaluations/candidate/summary.json",
...
def default_seed_candidate(fixture: FixtureMetadata) -> dict[str, str]:
    ...
    return {
        ...
        "proposal_generation_guidance": (
            ...
            "It must name the evidence model at a high level, including candidate versions, scoring records, judge records, ASI records, "
            "optimizer lineage, promotion evidence, and contract references. Ask at most 3-5 plain-language questions before drafting; "
            "include a draft eval contract for the user to confirm or correct."
        ),
        "design_generation_guidance": (
            ...
            "optimized metric, diagnostic metrics, guardrail metrics, promotion rule, regression tolerance, evidence ledger, run manifest, "
            "candidate registry, per-case scores, judge records, ASI records, rollout records, rollout persistence, judge output persistence, "
            "ASI persistence, optimizer lineage, and promotion decision."
        ),
        "spec_task_generation_guidance": (
            "Specs and tasks must cover fixture validation, eval dataset, candidate surface, rollout executor, "
            "scorers, ASI, GEPA optimization, compare, evidence ledger, runner invocation, judge records, ASI records, "
            "optimizer lineage, promotion evidence, negative fixtures, validation, and validation docs."
        ),
        "apply_generation_guidance": (
            "The apply plan must expose runnable generate, direct eval, compare, optimize, verify, and show-candidate "
            "operations. It must reuse existing target runtime helpers, block unsupported runtimes, persist an evidence ledger, "
            "record runner invocation, rollout persistence, judge output persistence, ASI persistence, optimizer lineage, and promotion decision."
        ),
        "scoring_asi_guidance": (
...
def contract_required_terms(artifact_type: str) -> list[str]:
    return list(CONTRACT_REQUIRED_TERMS.get(artifact_type, []))
...
def required_terms_for_artifact(fixture: FixtureMetadata, artifact_type: str) -> list[str]:
    ...
    base_terms = checks.get(artifact_type, defaults.get(artifact_type, []))
    return unique_terms([*base_terms, *contract_required_terms(artifact_type)])
    ...
    cases = [
        EvalCase(
            ...
            metadata={
                ...
                "evidence_terms": contract_required_terms("proposal"),
        ...
        EvalCase(
            ...
            metadata={
                ...
                "evidence_terms": contract_required_terms("design"),
        ...
        EvalCase(
            ...
            metadata={
                ...
                "evidence_terms": contract_required_terms("specs_tasks"),
        ...
        EvalCase(
            ...
            metadata={
                ...
                "evidence_terms": contract_required_terms("apply_plan"),
...
def semantic_required_concepts_scorer(case: EvalCase, rollout: RolloutResult) -> ScoreResult:
    ...
    evidence_terms = [str(term) for term in case.metadata.get("evidence_terms", []) if str(term).strip()]
    matched_evidence = [term for term in evidence_terms if term.lower() in actual]
    evidence_score = len(matched_evidence) / max(len(evidence_terms), 1) if evidence_terms else 1.0
    ...
    return ScoreResult(
        ...
        subscores={
            "semantic_concept_coverage": semantic_score,
            "missing_concepts": float(len(missing)),
            "lightweight_user_flow": ux_score,
            "evidence_contract_coverage": evidence_score,
        },
    )
...
The proposal names candidate versions, scoring records, judge records, ASI records, optimizer lineage, promotion evidence, and contract references at a high level. Detailed runner and evidence ledger layout belongs in design.
...
One rollout loads a candidate and eval case, starts the Claude Managed Agents session or deterministic fixture executor, captures trace data, runs the scorer, builds side_info and ASI, persists artifacts, and performs cleanup. Rollout persistence must write score, judge output, ASI, and rollout records for every candidate x case pair.

## Evidence Ledger

The run directory contains an evidence ledger with a run manifest, candidate registry, evaluation summaries, per-case scores, judge records, ASI records, rollout records, comparison records, optimizer lineage, leaderboard, and promotion decision. Judge output persistence and ASI persistence are required even when the grader is deterministic.
...
GEPA optimization uses optimize_candidate and optimize_anything with train/val splits, objective, background, max_metric_calls, command evidence, run logs, candidate outputs, optimizer lineage, and leaderboard records.
...
- Required evidence: summary.json, comparison.json, candidates.json, run_log.txt, side_info.json, command evidence, evidence ledger, optimizer lineage, and promotion decision.

## Verification Plan

Verification inspects the evidence ledger, runner invocation records, rollout persistence, judge output persistence, ASI persistence, optimizer lineage, comparison records, and promotion decision before reporting readiness.
...
The system SHALL validate the candidate surface, eval dataset, scorer behavior, Managed Agent runner invocation, evidence ledger, judge records, ASI records, GEPA optimization, optimizer lineage, promotion evidence, compare, negative fixtures, and validation documentation for {fixture.name}.
...
- [ ] Implement evidence ledger, runner invocation, judge records, ASI records, and GEPA optimization
- [ ] Implement compare, optimizer lineage, promotion evidence, validation, negative fixtures, and validation docs
...
Expose runnable commands for generate, direct eval, compare, optimize, verify, and show candidate. Reuse the target repo runtime helpers where possible. Persist an evidence ledger with run manifest, candidate registry, per-case scores, judge records, ASI records, rollout records, summary.json, comparison.json, candidates.json, run_log.txt, side_info.json, command evidence, and generated files.

Unsupported runtimes are blocked. Live Managed Agents require {REQUIRED_LIVE_ENV}; deterministic fixtures remain the default.

Optimizer acceptance must name the optimized metric, diagnostic metrics, guardrail metrics, promotion rule, regression tolerance, optimizer lineage, promotion decision, and required evidence. System-loop success proves machinery readiness only; agent-quality improvement requires criteria-tied comparison evidence.

Runner invocation, rollout persistence, judge output persistence, ASI persistence, optimizer lineage, and promotion decision evidence are required before verify can report readiness.

## Command Shape

python -m optimizespec.eval_validation generate --fixture {fixture.fixture_id} --run-dir runs/eval-validation/{fixture.fixture_id}
python -m optimizespec.eval_validation eval --fixture {fixture.fixture_id} --run-dir runs/eval-validation/{fixture.fixture_id}
python -m optimizespec.eval_validation compare --fixture {fixture.fixture_id} --run-dir runs/eval-validation/{fixture.fixture_id}
python -m optimizespec.eval_validation optimize --fixture {fixture.fixture_id} --max-metric-calls 1 --run-dir runs/eval-validation/{fixture.fixture_id}
python -m optimizespec.eval_validation verify --fixture {fixture.fixture_id} --run-dir runs/eval-validation/{fixture.fixture_id}
...
def update_evidence_manifest(
    ...
    *,
    ...
    previous = read_json_mapping(path)
    ...
    merged_candidate_ids = unique_terms([*(previous.get("candidate_ids", []) or []), *(candidate_ids or [])])
    merged_case_ids = unique_terms([*(previous.get("case_ids", []) or []), *(case_ids or [])])
    payload = {
        "run_id": previous.get("run_id") or run_dir.name or fixture.fixture_id,
        "fixture": fixture.fixture_id,
        "target_agent": fixture.name,
        "runtime": fixture.runtime,
        "commands": commands,
        "candidate_ids": merged_candidate_ids,
        "case_ids": merged_case_ids,
...
def candidate_registry_record(candidate_id: str, candidate: dict[str, str], *, parent_id: str | None, creation_reason: str) -> dict[str, Any]:
    return {
...
def update_candidate_registry(
    ...
    *,
    ...
    creation_reason: str = "validation command",
) -> None:
    ...
    for candidate_id, candidate in candidates.items():
        records[candidate_id] = candidate_registry_record(
            candidate_id,
            candidate,
            parent_id=(parent_ids or {}).get(candidate_id),
            creation_reason=creation_reason,
        )
    write_json(path, {"candidates": [records[key] for key in sorted(records)]})


def copy_json_record(source: Path, target: Path, fallback: dict[str, Any] | None = None) -> None:
    if source.exists():
...
def write_eval_evidence(run_dir: Path, *, summary: dict[str, Any], source_dir: Path, candidate_id: str) -> None:
    eval_root = evidence_root(run_dir) / "evaluations" / candidate_id
    ...
    for item in summary.get("cases", []):
        ...
        if not case_id:
            continue
        source_case = source_dir / "rollouts" / candidate_id / case_id
        ...
        score_payload = {
            ...
            "criterion": item.get("split"),
            "scorer_identity": "eval-validation",
            "higher_is_better": True,
            "errors": item.get("errors", []),
        }
        copy_json_record(source_case / "score.json", target_case / "score.json", score_payload)
        judge_payload = {
            "candidate_id": candidate_id,
            "case_id": case_id,
            "grader_type": "deterministic",
            "rubric_or_criterion": item.get("split"),
            "structured_output": {"score": item.get("score"), "feedback": item.get("feedback")},
        ...
        copy_json_record(source_case / "side_info.json", target_case / "side_info.json")
        copy_json_record(source_case / "rollout.json", target_case / "rollout.json")
...
def write_optimizer_evidence(
    ...
    *,
    ...
    seed_id = "seed"
    selected_id = "optimizer-selected"
    update_candidate_registry(
        run_dir,
        {seed_id: seed_candidate, selected_id: best_candidate},
        parent_ids={selected_id: seed_id},
        creation_reason="GEPA optimization",
    )
    lineage = {
        "fixture": fixture.fixture_id,
        ...
        "events_source": "optimize/run_log.txt",
        "candidates_source": "optimize/candidates.json",
        "mutated_fields": sorted(best_candidate),
    }
    leaderboard = {
        "objective_metric": "semantic_concept_coverage",
        "candidates": [
            {"candidate_id": seed_id, "selected": False, "source": "seed"},
            {"candidate_id": selected_id, "selected": True, "source": "optimizer"},
        ],
    }
    event = {
        "event": "optimizer_result_recorded",
        "seed_candidate_id": seed_id,
        "selected_candidate_id": selected_id,
        "max_metric_calls": max_metric_calls,
    }
    promotion = {
        ...
        "reason": "Deterministic validation records optimizer output; manual review is required before promotion.",
        "objective_metric_result": "recorded",
        "guardrail_result": "manual_review_required",
        "manual_review_required": True,
        "rollback_path": seed_id,
    }
...
def required_artifacts(fixture: FixtureMetadata) -> list[str]:
    ...
    if isinstance(value, list) and value:
        return unique_terms([*[str(item) for item in value], *EVIDENCE_REQUIRED_ARTIFACTS])
...
def validate_evidence_ledger(run_dir: Path) -> list[str]:
    errors: list[str] = []
    root = evidence_root(run_dir)
    for relative in EVIDENCE_REQUIRED_ARTIFACTS:
        if not (run_dir / relative).exists():
            errors.append(f"{relative}: missing")

    manifest = read_json_mapping(root / "manifest.json")
    if manifest:
        for key in ("run_id", "fixture", "runtime", "candidate_ids", "case_ids"):
            if key not in manifest:
                errors.append(f"evidence/manifest.json: missing {key!r}")
        if not isinstance(manifest.get("candidate_ids"), list):
            errors.append("evidence/manifest.json: candidate_ids must be a list")
        if not isinstance(manifest.get("case_ids"), list):
            errors.append("evidence/manifest.json: case_ids must be a list")

    registry = read_json_mapping(root / "candidate-registry.json")
    candidates = registry.get("candidates") if registry else None
    if candidates is not None:
        if not isinstance(candidates, list) or not candidates:
            errors.append("evidence/candidate-registry.json: candidates must be a non-empty list")
        else:
            for item in candidates:
                if not isinstance(item, dict) or not item.get("candidate_id") or not isinstance(item.get("mutable_fields"), list):
                    errors.append("evidence/candidate-registry.json: each candidate needs candidate_id and mutable_fields")
                    break

    case_dirs = sorted((root / "evaluations").glob("*/cases/*")) if (root / "evaluations").exists() else []
    if not case_dirs:
        errors.append("evidence/evaluations: missing per-case evidence")
    for case_dir in case_dirs:
        display = case_dir.relative_to(run_dir)
        for name in ("score.json", "judge.json", "side_info.json", "rollout.json"):
            if not (case_dir / name).exists():
                errors.append(f"{display}/{name}: missing")
        if (case_dir / "score.json").exists():
            try:
                score_payload = json.loads((case_dir / "score.json").read_text(encoding="utf-8"))
                if "score" not in score_payload or "feedback" not in score_payload:
                    errors.append(f"{display}/score.json: missing score or feedback")
            except Exception as exc:
                errors.append(f"{display}/score.json: {exc}")
        if (case_dir / "judge.json").exists():
            try:
                judge_payload = json.loads((case_dir / "judge.json").read_text(encoding="utf-8"))
                if "grader_type" not in judge_payload or "structured_output" not in judge_payload:
                    errors.append(f"{display}/judge.json: missing grader_type or structured_output")
            except Exception as exc:
                errors.append(f"{display}/judge.json: {exc}")
        if (case_dir / "side_info.json").exists():
            try:
                validate_side_info(json.loads((case_dir / "side_info.json").read_text(encoding="utf-8")))
            except Exception as exc:
                errors.append(f"{display}/side_info.json: {exc}")
        if (case_dir / "rollout.json").exists():
            try:
                rollout_payload = json.loads((case_dir / "rollout.json").read_text(encoding="utf-8"))
                for key in ("trajectory", "runtime_ids", "errors"):
                    if key not in rollout_payload:
                        errors.append(f"{display}/rollout.json: missing {key!r}")
            except Exception as exc:
                errors.append(f"{display}/rollout.json: {exc}")

    comparison = read_json_mapping(root / "comparisons" / "comparison.json")
    if comparison:
        try:
            validate_comparison(comparison)
        except Exception as exc:
            errors.append(f"evidence/comparisons/comparison.json: {exc}")

    for relative in ("optimizer/lineage.json", "optimizer/leaderboard.json", "promotion.json"):
        payload = read_json_mapping(root / relative)
        if payload and relative == "optimizer/lineage.json" and "selected_candidate_id" not in payload:
            errors.append("evidence/optimizer/lineage.json: missing selected_candidate_id")
        if payload and relative == "optimizer/leaderboard.json" and "candidates" not in payload:
            errors.append("evidence/optimizer/leaderboard.json: missing candidates")
        if payload and relative == "promotion.json" and "decision" not in payload:
            errors.append("evidence/promotion.json: missing decision")
    events_path = root / "optimizer" / "events.jsonl"
    if events_path.exists():
        lines = [line for line in events_path.read_text(encoding="utf-8").splitlines() if line.strip()]
        if not lines:
            errors.append("evidence/optimizer/events.jsonl: empty")
        for line in lines:
            try:
                json.loads(line)
            except Exception as exc:
                errors.append(f"evidence/optimizer/events.jsonl: {exc}")
                break
    return errors
...
def validate_run_artifacts(run_dir: Path) -> list[str]:
    ...
    errors.extend(validate_evidence_ledger(run_dir))
    return errors
...
def cmd_generate(fixture_id: str, run_dir: str, candidate_path: str | None, live: bool) -> int:
    ...
    update_evidence_manifest(run_path, fixture, command="generate", candidate_ids=["seed"])
    update_candidate_registry(run_path, {"seed": candidate}, creation_reason="generated seed candidate")
...
def cmd_eval(fixture_id: str, run_dir: str, candidate_path: str | None, cases_path: str | None, timeout_seconds: float, skip_system_loop: bool, live: bool) -> int:
    ...
    update_evidence_manifest(run_path, fixture, command="eval", candidate_ids=["candidate"], case_ids=[case.case_id for case in cases])
    update_candidate_registry(run_path, {"candidate": candidate}, creation_reason="direct eval candidate")
    write_eval_evidence(run_path, summary=summary, source_dir=run_path / "eval", candidate_id="candidate")
...
def cmd_compare(
    ...
    update_candidate_registry(
        ...
        creation_reason="comparison candidate",
    )
    write_eval_evidence(run_path, summary=comparison["baseline"], source_dir=run_path / "compare" / "baseline", candidate_id="baseline")
    write_eval_evidence(run_path, summary=comparison["candidate"], source_dir=run_path / "compare" / "candidate", candidate_id="candidate")
...
def cmd_optimize(
    ...
    update_evidence_manifest(
        ...
        fixture,
        command="optimize",
        candidate_ids=["seed", "optimizer-selected"],
        case_ids=[case.case_id for case in cases],
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="python -m optimizespec.eval_validation")
