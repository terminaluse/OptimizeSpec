...
Keep loading progressive: use only the references needed for the current phase, infer the target runtime from repo inspection when possible, and cite the contract names in generated artifacts instead of copying long contract prose into user-facing documents.
