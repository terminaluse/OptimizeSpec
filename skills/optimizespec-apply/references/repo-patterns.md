...
Use these patterns when adapting OptimizeSpec to a Claude Managed Agents project. They are intentionally described here instead of linking to files outside this installed skill folder.
...
Reuse the project's existing agent factories, environment setup, session creation, event streaming, output collection, and cleanup conventions. Do not create a parallel Managed Agents path when the project already has one.
