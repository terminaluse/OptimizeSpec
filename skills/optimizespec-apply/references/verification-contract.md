...
When live credentials and preview access are available, run the smallest useful Managed Agents smoke. Inspect run manifest, rollout records, per-case scores, judge output when present, ASI, optimizer lineage, and promotion decision before reporting readiness.
