...
class ManagedAgentRuntime:
    def __init__(
        ...
        *,
        ...
        self.skill_registry = LocalSkillRegistry(skill_registry_path or Path(".optimizespec") / "skill_registry.json")
    ...
    def run_task(
        ...
        *,
        ...
        agent = self._create_agent(
            name=f"optimizespec-agent-{suffix}",
        ...
        environment = self.client.beta.environments.create(
            name=f"optimizespec-env-{suffix}",
        ...
        session_kwargs: dict[str, Any] = {
            ...
            "title": f"optimizespec-{task.task_id}",
            "metadata": {
        ...
        with self.client.beta.sessions.events.stream(session_id=session.id, betas=STREAM_BETAS) as stream:
            ...
            for event in stream:
                ...
                if event.type in {"agent.tool_use", "agent.tool_result"}:
                ...
                elif event.type in {"span.outcome_evaluation_start", "span.outcome_evaluation_ongoing"}:
                    outcome_evaluation_in_progress = True
                    ...
                    if outcome_evaluation_in_progress:
                        continue
    ...
    def _create_subagents(self, subagents: tuple[SubagentSpec, ...], *, suffix: str) -> list[dict[str, Any]]:
        ...
        for index, subagent in enumerate(subagents, start=1):
            ...
            created = self._create_agent(
                name=f"optimizespec-subagent-{index}-{suffix}",
